...
class Utumba():
    def __init__(self,url,query):
        ...
        while self.data == '': # посылаем запрос в Ютуб
            try:
    ...
    def update(self):
        ...
        if self.full.is_set()==False:
            self.label.configure(text=f'Запрос={self.query} Threads={self.counter} Images={self.shown} Время:{round(now,1)}')
    ...
    def get_img_url(self,num):
        ...
        if pos_in_content != -1:
            ...
            if len(url_str)<100:return url_str
            else:return ''
        else:
            return 'end'
